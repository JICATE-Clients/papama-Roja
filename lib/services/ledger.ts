...
/**
 * The two POOL streams are donated value awaiting a meal. Neither is revenue,
 * and the distinction is not cosmetic: booking either as revenue records donated
 * money as income to pApAmA, and every transparency figure and CSR statement
 * downstream inherits the error.
 *
 *   `special_care_pool` (A-3 / CD §D-8) — the ₹100-minus-meal surplus from a
 *       Special Care redemption. Funds future special-care meals.
 *   `meal_pool` (F-4 / B-03) — standard-token forfeited difference and expired
 *       token value. Funds another meal for anyone.
 */
